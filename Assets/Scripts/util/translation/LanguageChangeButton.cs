...
public class LanguageChangeButton : MonoBehaviour, IPointerClickHandler {
    ...
    private static void SetNewGameLanguage(string lang) {
        ...
        if (!LanguageSelect.languageSelectDisabled) {
            ...
            NotificationHandler.ShowNotification_String($"{langCode}: {I18nTextTranslator.SetTranslatedText("languagerestartnotification")}", NotificationHandler.notificationColorGreen);
            ...
            NotificationHandler.ShowNotification_String($"{langCode}: {I18nTextTranslator.SetTranslatedText("languagetemporarilydisabled")}", NotificationHandler.notificationColorYellow);
